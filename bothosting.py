...
# --- Configuration ---
TOKEN = '8665745880:AAFK_kHuUQ69rzgk7f8SpWEVSD4podEuIEk' 
OWNER_ID = 7324148028
ADMIN_ID = 7324148028 
YOUR_USERNAME = '@tobigotfans' 
UPDATE_CHANNEL = 'https://t.me/teritangtordunga'
...
def check_user_joined_channel(user_id, channel_username='godofcomsfck'):
    """Check if user has joined the channel"""
    try:
        # Try to get user status in channel
        member_status = bot.get_chat_member(f'@{channel_username}', user_id)
        # Check if user is member (not left, kicked, etc)
...
def _logic_statistics(message):
    ...
    # Check if user joined the channel (required for status)
    if not check_user_joined_channel(user_id):
        # User hasn't joined, send them the channel link
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton('📢 Join Channel', url=UPDATE_CHANNEL))
        markup.add(types.InlineKeyboardButton('✅ I Joined', callback_data='verify_joined'))
        bot.reply_to(message, 
            "🔔 Please join our channel first to use this command:\n\n"
            f"{UPDATE_CHANNEL}\n\n"
            "After joining, click 'I Joined' button below ✅", 
            reply_markup=markup)
        return
    
    # User has joined, show statistics
    ...
    stats_msg_base = (f"✅ Status Successful!\n\n"
                      f"📊 Bot Statistics:\n\n"
    ...
    bot.reply_to(message, stats_msg, reply_markup=create_reply_keyboard_main_menu(user_id))
...
def handle_callbacks(call):
    ...
    if bot_locked and user_id not in admin_ids and data not in ['back_to_main', 'speed', 'stats', 'verify_joined']:
    ...
    try:
        ...
        elif data == 'verify_joined': verify_channel_joined_callback(call)
...
def verify_channel_joined_callback(call):
    """Handle the 'I Joined' button callback"""
    user_id = call.from_user.id
    bot.answer_callback_query(call.id)
    
    if check_user_joined_channel(user_id):
        # User successfully joined, show statistics
        bot.edit_message_text(
            "✅ Great! Channel verified.\n\n"
            "Showing statistics...",
            call.message.chat.id,
            call.message.message_id
        )
        # Create a temporary message object to call _logic_statistics
        temp_message = call.message
        _logic_statistics(temp_message)
    else:
        ...
        markup.add(types.InlineKeyboardButton('✅ I Joined', callback_data='verify_joined'))
        bot.edit_message_text(
            "⚠️ It seems you haven't joined the channel yet.\n\n"
            "Please join first and then click 'I Joined' again ✅",
            call.message.chat.id,
            call.message.message_id,
            reply_markup=markup
        )
